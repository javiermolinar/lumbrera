...
.claude -> .agents    # Claude-compatible view of bundled skills
...
- Generate `AGENTS.md`.
- Create `CLAUDE.md` as a symlink to `AGENTS.md` for Claude compatibility.
- Install the Lumbrera operation skills under `.agents/skills/lumbrera-ingest/`, `.agents/skills/lumbrera-query/`, and `.agents/skills/lumbrera-lint/`.
- Create `.claude` as a symlink to `.agents` so Claude can discover the same skills.
...
Future semantic lint can use LLMs and embeddings to suggest duplicate concepts, stale claims, contradictions, unsupported synthesis, missing conceptual coverage, and pages that should be merged. Those checks should produce suggestions, not mandatory rewrites. Deterministic checks such as missing links remain Lumbrera CLI work.
...
First integrations:
- Generic `AGENTS.md` generated by `init`, with `CLAUDE.md` symlinked to it.
- Generic `.agents/skills/lumbrera-ingest/SKILL.md`, `.agents/skills/lumbrera-query/SKILL.md`, and `.agents/skills/lumbrera-lint/SKILL.md` generated by `init`, with `.claude` symlinked to `.agents` for Claude.
- Pi-discoverable skills for ingest, query, and lint workflows.
...
16. Generate `AGENTS.md` during init and symlink `CLAUDE.md` to it.
17. Generate the Lumbrera ingest, query, and lint skills under `.agents/skills` and symlink `.claude` to `.agents`.
18. Ship Pi-discoverable ingest, query, and lint skills.
19. Dogfood with Pi on a real Lumbrera repo.
20. Add tests for init, write, append, generated files, checksum drift, deterministic lint, hooks, sync, and conflict cases.
...
CLAUDE.md symlink
.agents/skills/lumbrera-ingest/SKILL.md generation
.agents/skills/lumbrera-query/SKILL.md generation
.agents/skills/lumbrera-lint/SKILL.md generation
.claude symlink
```
