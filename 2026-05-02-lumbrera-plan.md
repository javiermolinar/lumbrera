...
.agents/skills/       # bundled agent skills, including the Lumbrera contract
...
- Install the Lumbrera skill under `.agents/skills/lumbrera/SKILL.md`.
- Create `.claude` as a symlink to `.agents` so Claude can discover the same skill.
...
- Generic `.agents/skills/lumbrera/SKILL.md` generated by `init`, with `.claude` symlinked to `.agents` for Claude.
...
17. Generate the Lumbrera skill under `.agents/skills` and symlink `.claude` to `.agents`.
18. Ship a Pi skill.
...
CLAUDE.md symlink
.agents/skills/lumbrera/SKILL.md generation
.claude symlink
```
